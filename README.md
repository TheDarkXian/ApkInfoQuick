...
- 支持 AAB：通过 bundletool 转换为 universal APK 后复用 APK 解析链路。
- 高密度信息布局，适合日常 APK 检查。
- 应用图标预览与导出。
- 当前标签内容可复制为纯文本或 JSON。
- 提供 CLI，适合脚本和批处理。
...
- 原生 AAB 解析支持，减少对 bundletool 和 Java 的依赖。
- 更完整的签名验证。
- 更多导出格式。
...
- AAB support through bundletool conversion to a universal APK.
...
- Native AAB parsing to reduce the dependency on bundletool and Java.
