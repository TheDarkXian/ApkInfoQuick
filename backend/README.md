...
- Parses AAB by converting it to a universal APK with `bundletool.jar`, then reusing the APK parser.
...
- `--recursive`: recursively scan directories for `.apk` and `.aab` files
...
- AAB parsing requires Java at runtime because `bundletool.jar` is a Java application.
