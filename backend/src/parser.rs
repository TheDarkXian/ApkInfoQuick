...
mod warnings {
    ...
    pub const AAB_CONVERTED_BY_BUNDLETOOL: &str = "AAB_CONVERTED_BY_BUNDLETOOL";
    pub const AAB_BUNDLETOOL_NOT_FOUND: &str = "AAB_BUNDLETOOL_NOT_FOUND";
    pub const AAB_JAVA_NOT_FOUND: &str = "AAB_JAVA_NOT_FOUND";
    pub const AAB_BUNDLETOOL_FAILED: &str = "AAB_BUNDLETOOL_FAILED";
    pub const AAB_UNIVERSAL_APK_NOT_FOUND: &str = "AAB_UNIVERSAL_APK_NOT_FOUND";
...
mod aab_parser {
    ...
    pub fn convert_to_universal_apk(
        aab_path: &Path,
        warnings: &mut Vec<String>,
    ) -> Result<PathBuf, BackendError> {
        let Some(bundletool) = find_bundletool() else {
            super::push_warning(warnings, super::warnings::AAB_BUNDLETOOL_NOT_FOUND);
            return Err(BackendError::AabConvertFailed);
        };
        let Some(java) = find_java() else {
            super::push_warning(warnings, super::warnings::AAB_JAVA_NOT_FOUND);
            return Err(BackendError::AabConvertFailed);
        };

        let work_dir = build_work_dir(aab_path)?;
        fs::create_dir_all(&work_dir).map_err(|_| BackendError::AabConvertFailed)?;
        let apks_path = work_dir.join("bundle.apks");
        let universal_apk = work_dir.join("universal.apk");
        let _ = fs::remove_file(&apks_path);
        let _ = fs::remove_file(&universal_apk);

        let status = Command::new(java)
            .arg("-jar")
            .arg(bundletool)
            .arg("build-apks")
            .arg(format!("--bundle={}", aab_path.to_string_lossy()))
            .arg(format!("--output={}", apks_path.to_string_lossy()))
            .arg("--mode=universal")
            .arg("--overwrite")
            .status()
            .map_err(|_| BackendError::AabConvertFailed)?;

        if !status.success() || !apks_path.is_file() {
            super::push_warning(warnings, super::warnings::AAB_BUNDLETOOL_FAILED);
            return Err(BackendError::AabConvertFailed);
        }

        extract_universal_apk(&apks_path, &universal_apk, warnings)?;
        Ok(universal_apk)
    }

    fn extract_universal_apk(
        apks_path: &Path,
        output_path: &Path,
        warnings: &mut Vec<String>,
    ) -> Result<(), BackendError> {
        let file = File::open(apks_path).map_err(|_| BackendError::AabConvertFailed)?;
        let mut archive = ZipArchive::new(file).map_err(|_| BackendError::AabConvertFailed)?;
        let entry_name = (0..archive.len())
            ...
            .find(|name| {
                name.eq_ignore_ascii_case("universal.apk") || name.ends_with("/universal.apk")
            });

        let Some(entry_name) = entry_name else {
            super::push_warning(warnings, super::warnings::AAB_UNIVERSAL_APK_NOT_FOUND);
            return Err(BackendError::AabConvertFailed);
        };
        ...
        entry
            ...
            .map_err(|_| BackendError::AabConvertFailed)?;
        fs::write(output_path, bytes).map_err(|_| BackendError::AabConvertFailed)?;
        Ok(())
    }
    ...
    fn build_work_dir(aab_path: &Path) -> Result<PathBuf, BackendError> {
        let stem = aab_path
            .file_stem()
            .and_then(OsStr::to_str)
            .unwrap_or("bundle")
            ...
            .collect::<String>();
        let digest = Sha256::digest(aab_path.to_string_lossy().as_bytes());
        let suffix = format!("{:x}", digest);
        Ok(env::temp_dir()
            .join("apk-info-aab")
            .join(format!("{stem}-{}", &suffix[..12])))
    }
